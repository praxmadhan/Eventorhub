...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (limitToggle && limitFields) {
        function toggleLimitFields() {
            var isVisible = limitToggle.checked;
            limitFields.classList.toggle('visible', isVisible);
            setFieldsDisabled(limitFields, !isVisible);
            if (!isVisible) {
    ...
    if (paidToggle && paidFields) {
        function togglePaymentFields() {
            var isVisible = paidToggle.checked;
            paidFields.classList.toggle('visible', isVisible);
            setFieldsDisabled(paidFields, !isVisible);
            if (!isVisible) {
    ...
    // ─── Toggle Team Fields ──────────────────────────────────
    const teamToggle = document.getElementById('is_team_event');
    const teamFields = document.getElementById('team-fields');

    if (teamToggle && teamFields) {
        function toggleTeamFields() {
            var isVisible = teamToggle.checked;
            teamFields.classList.toggle('visible', isVisible);
            setFieldsDisabled(teamFields, !isVisible);
            if (!isVisible) {
                const sizeInput = document.getElementById('team_size');
                if (sizeInput) sizeInput.value = '1';
            }
        }
        teamToggle.addEventListener('change', toggleTeamFields);
        toggleTeamFields(); // Initialize on page load
    }
    ...
    document.querySelectorAll('form[data-validate]').forEach(function (form) {
        form.addEventListener('submit', function (e) {
            ...
            // Check required fields (skip disabled and hidden fields)
            form.querySelectorAll('[required]').forEach(function (field) {
                if (field.disabled) return;
                var parent = field.closest('.toggle-field');
