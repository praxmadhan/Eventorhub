...
def init_db():
    ...
    try:
        c.execute('ALTER TABLE events ADD COLUMN is_team_event INTEGER DEFAULT 0')
    except sqlite3.OperationalError:
        pass
    try:
        c.execute('ALTER TABLE events ADD COLUMN team_size INTEGER DEFAULT 1')
    except sqlite3.OperationalError:
        pass

    # Migrate registrations table
    try:
        c.execute('ALTER TABLE registrations ADD COLUMN transaction_id TEXT DEFAULT ""')
    except sqlite3.OperationalError:
        pass
    try:
        c.execute('ALTER TABLE registrations ADD COLUMN team_name TEXT DEFAULT ""')
    except sqlite3.OperationalError:
        pass
    try:
        c.execute('ALTER TABLE registrations ADD COLUMN team_members TEXT DEFAULT ""')
    except sqlite3.OperationalError:
        pass
...
def send_email(to, subject, body):
    """Send email using Flask-Mail. Prompts user if not configured."""
    if not MAIL_ENABLED:
        print("[!] Cannot send email: Flask-Mail is not installed.")
        return False
        
    if not app.config.get('MAIL_USERNAME'):
        print(f"[SIMULATED EMAIL] Configure MAIL_USERNAME/PASSWORD to send real emails.")
        print(f"To: {to} | Sub: {subject}")
        # We return True so the success flash still shows up, but we also flash a warning
        # if there's a request context
        try:
            from flask import request
            if request:
                flash("Note: Email is not fully configured, so the mail was just simulated in the console.", "warning")
        except:
            pass
        return True
    ...
    try:
        ...
        try:
            from flask import request
            if request:
                flash(f"Failed to send email. Check your SMTP settings. Error: {e}", "danger")
        except:
            pass
        return False
...
def create_razorpay_order():
    ...
    amount = int(event['price'] * 100)  # Amount in paise
    
    try:
        order = razorpay_client.order.create({
            'amount': amount,
            'currency': 'INR',
            'receipt': f'receipt_{event_id}_{session["user_id"]}'
        })
        return jsonify({
            'order_id': order['id'],
            'amount': amount,
...
def register_event(event_id):
    ...
    # For paid events, verify Razorpay payment signature
    ...
    if event['is_paid']:
        ...
        try:
            ...
            payment_status = 'verified'  # Auto-verified via Razorpay
        except razorpay.errors.SignatureVerificationError:
            flash('Invalid payment signature. Transaction failed.', 'danger')
            conn.close()
            return redirect(url_for('events'))

    # Insert registration record
    conn.execute(
        'INSERT INTO registrations (user_id, event_id, payment_status, transaction_id, team_name, team_members) VALUES (?, ?, ?, ?, ?, ?)',
        (session['user_id'], event_id, payment_status, razorpay_payment_id, team_name, team_members)
    )
    conn.commit()
...
def add_event():
    ...
    if request.method == 'POST':
        ...
        max_participants = request.form.get('max_participants', 0, type=int) if limit_enabled else 0
        is_paid = 1 if request.form.get('is_paid') else 0
        price = request.form.get('price', 0, type=int) if is_paid else 0
        upi_id = request.form.get('upi_id', '').strip() if is_paid else ''
        is_team_event = 1 if request.form.get('is_team_event') else 0
        team_size = request.form.get('team_size', 1, type=int) if is_team_event else 1
        ...
        conn.execute('''INSERT INTO events
            (event_name, description, date, time, venue, poster,
             limit_enabled, max_participants, is_paid, price, upi_id, payment_qr, is_team_event, team_size)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (event_name, description, date, time, venue, poster_filename,
             limit_enabled, max_participants, is_paid, price, upi_id, qr_filename, is_team_event, team_size))
        conn.commit()
...
def edit_event(event_id):
    ...
    if request.method == 'POST':
        ...
        conn.execute('''UPDATE events SET
            ...
            WHERE event_id=?''',
            (event_name, description, date, time, venue, poster_filename,
             limit_enabled, max_participants, is_paid, price, upi_id, qr_filename, is_team_event, team_size, event_id))
        conn.commit()
...
def participants(event_id):
    ...
    participant_list = conn.execute('''
        SELECT u.name, u.email, u.department, u.batch, u.college_name,
               r.payment_status, r.reg_id, r.transaction_id, r.team_name, r.team_members
...
def download_participants(event_id):
    ...
    participant_list = conn.execute('''
        SELECT u.name as "Student Name", u.email as "Email",
               ...
               r.team_name as "Team Name", r.team_members as "Team Members",
               e.event_name as "Event Name", e.date as "Date",
               r.payment_status as "Payment Status", r.transaction_id as "Transaction ID"
        FROM registrations r
        JOIN users u ON r.user_id = u.id
    ...
    df = pd.DataFrame(data) if data else pd.DataFrame(
        columns=["Student Name", "Email", "Department", "Batch", "College", "Team Name", "Team Members",
                 "Event Name", "Date", "Payment Status", "Transaction ID"]
    )
...
def reject_payment(reg_id):
    ...
    conn.execute('DELETE FROM registrations WHERE reg_id = ?', (reg_id,))
    conn.commit()
    conn.close()
    flash('Payment rejected. Student has been notified and can register again.', 'warning')
